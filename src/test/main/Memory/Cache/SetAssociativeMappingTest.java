...
public class SetAssociativeMappingTest {
    ...
    public void test03() {
        ...
        for (int i = 0; i < 4; i++) {
            if (i == 1) continue;
            String pAddrHit = transformer.DecimalToBinary("" + i).substring(18, 32) + "000000000000000000";
            dataRead = cache.read(pAddrHit, 1024);
            assertArrayEquals(dataHit, dataRead);
        }
        for (int i = 0; i < 4; i++) {
            if (i == 0) continue;
            String pAddrHit = transformer.DecimalToBinary("" + i).substring(18, 32) + "000000010000000000";
            dataRead = cache.read(pAddrHit, 1024);
            assertArrayEquals(dataHit, dataRead);
        }
